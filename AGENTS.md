...
- **Supply chain**: `[tool.uv] exclude-newer = "7 days"` in `pyproject.toml` defers new third-party dependency releases by 7 days; `exclude-newer-package` exempts the first-party mloda, mloda-testing, and mloda-registry packages from that cooldown (rolling `"0 days"`), so their latest releases are picked up immediately. `override-dependencies` additionally forces `pymdown-extensions>=11.0.1` past marimo's `<11` pin, because the fix for the b64 path-traversal advisory only ships in 11.0 and a constraint cannot beat an upper pin. Do not edit these without a reason.
